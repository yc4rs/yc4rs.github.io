 ...
 *
 * 【API】
 *   hit   — 寫入一列，回傳 { ok, today, week }（today 已含本筆）
 *   patch — 依 visit_ts 補 device_fp、public_ip（不影響筆數）
 *
...
const LOG_HEADERS = ["visit_ts", "timestamp", "device_id", "device_fp", "public_ip"];
...
function doGet(e) {
  ...
  if (action === "hit") {
    ...
    try {
      recordVisit(e);
      return jsonResponse(buildHitResponse());
    } finally {
      lock.releaseLock();
    }
  }
...
function recordVisit(e) {
  ...
  sheet
    .getRange(row, 1, 1, LOG_COL_COUNT)
    .setValues([
      [
...
function patchVisitDetails(e) {
  const params = (e && e.parameter) || {};
  const visitTs = parseVisitTs(params);
  if (!visitTs) return;

  const deviceFp = sanitizeCell(params.device_fp, 64);
  ...
  if (deviceFp) {
    sheet.getRange(row, 4).setValue(deviceFp);
  }
  if (publicIp) {
